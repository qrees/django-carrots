...
Są to zwykłe nazwy, które widzieliśmy wcześniej. Są tylko dwie różnice:

Po pierwsze, nazwy argumentów każdej funkcji są definiowane w czasie wywołania
każdej funkcji, a Python przypisuje im odpowiednie wartości.

Po drugie, nazwy argumentów nie są dostępne poza funkcja, gdyż są tworzone w czasie
wywołania funkcji i niszczone po jej zakończeniu. Oznacza to, że jeżeli spróbujesz
uzyskać dostęp do wartości ``n`` zdefiniowanej w funkcji :func:`plus_five`, poza
tą funkcją, dostaniesz błąd:
...
Python sprząta po sobie po wykonaniu funkcji.
...
Instancja to nowa wartość typu opisanego przez klasę.
